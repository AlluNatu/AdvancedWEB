...
function MyContainer() {
    ...
    const addItem = () => {
          ...
          setItems([...items, madeItem]);            // Jokaisella state componentilla on oma funktion, tässä se lisää listaan ...items uuden itemin (...items koska silloin ottaa huomioon kaikki listassa olevat objektit)
          setNewItem('');                       // Laittaa tekstin takaisin tyhjäksi.
      };

      function handleChange(e: React.ChangeEvent<HTMLTextAreaElement>) {     // Pakko oli antaa tolle e variablelle tyyppi ku anto error :( (Tyyppi on otettu suaraa vsc quick fix :D:D:D)                
